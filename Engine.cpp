...
void my_system::modify_profile() // todo 修改已登录用户权限
{
    para temp_para(command_stream) ;
    if ( !check_login(temp_para.c) ) { fail(); return ; }
    IndexKey user_key(temp_para.u) ;
...
void my_system::query_ticket()
{
    ...
    sort(available_ride.begin(),available_ride.end(),less<pair<ride,train>>()) ;
...
void my_system::query_transfer()
{
    ...
    for ( int i = 0 ; i < train_key.size() ; i++ ){
        ...
        temp_vec.clear() ;
    } // 收集所有出发站的 train // todo temp_date 变量名混乱
    train_key.clear() ;
...
void my_system::refund_ticket() // todo line 265 refund 失败
{
    para temp_para(command_stream) ;
