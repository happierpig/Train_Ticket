...
station &station::operator=( const station &other )
{
    ...
    location_pos = other.location_pos ;
    strcpy( trainID , other.trainID ) ;
    strcpy( station_name , other.station_name ) ;
    station_set_off = other.station_set_off ;
    station_arrive_in = other.station_arrive_in ;
    sale_begin = other.sale_begin ;
    sale_end = other.sale_end ;
    price_add_up = other.price_add_up ;
    return *this ;
}
...
bool station::in_sale( date purchase_day )
{
    date temp_date_1 = purchase_day , temp_date_2 = purchase_day ;
    temp_date_1.become_last_minute() ; // 一天的最后一刻搭车
    temp_date_2.become_first_minute();  // 一天的第一刻搭车
    temp_date_1 = temp_date_1 - ( station_set_off - sale_begin ) ; // all_departure[i] = all_set_off[i] - all_set_off[1]
    temp_date_2 = temp_date_2 - ( station_set_off - sale_begin ) ;
    if ( temp_date_1 < sale_begin  ) return false ;
    if ( temp_date_2 > sale_end  ) return false ;
    return true ;
}

date station::get_date_index(date purchase_day)
{
    date temp_date = purchase_day ;
    temp_date.get_other_time(station_set_off) ;
    temp_date = temp_date - ( station_set_off - sale_begin ) ;
    temp_date.become_first_minute() ;
    return temp_date ;
}

station::station( int t_pos , int l_pos , train &temp_train )
{
    train_pos = t_pos ;
    location_pos = l_pos ;
    strcpy(trainID,temp_train.trainID) ;
    strcpy(station_name,temp_train.all_station[l_pos]) ;
    station_set_off = temp_train.all_set_off[l_pos] ;
    station_arrive_in = temp_train.all_arrive_in[l_pos] ;
    sale_begin = temp_train.sale_begin ;
    sale_end = temp_train.sale_end ;
    price_add_up = temp_train.all_price[l_pos] ;
}
